                                         ...
                                         Velocity& cmd_vel)
{
  ...
  std::vector<Pose2D> pruned_global_plan = cutGlobalPlan(global_plan, size_x, size_y, robot_pose);
...
std::vector<Pose2D> DWAPlanner::cutGlobalPlan(const std::vector<Pose2D>& global_plan, const int& size_x, const int& size_y, const Pose2D& robot_pose){
  std::vector<Pose2D> pruned_global_plan;
  for (std::vector<Pose2D>::const_iterator it=global_plan.begin(); it!=global_plan.end(); it++){
    if (abs(it->x-robot_pose.x)>size_x/2.0 && abs(it->y-robot_pose.y)>size_y/2.0){
      break;
    }
