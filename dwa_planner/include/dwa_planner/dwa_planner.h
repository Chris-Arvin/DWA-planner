...
public:
  ...
  /**
   * @brief cut up global plan into local map
   * @param global_plan: global path
   * @param paramsOfLocalMap: size_x, size_y
   * @param robot_pose: current pose of robot
   * @return cutted global plan, that is, local plan
   */
  std::vector<Pose2D> cutGlobalPlan(const std::vector<Pose2D>& global_plan, const int& size_x, const int& size_y, const Pose2D& robot_pose);
  /**
